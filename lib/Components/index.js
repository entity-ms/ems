 ...
 * @todo
 */
Components.prototype.info = function (name) {
  'use strict';

  if (this._instances[name] !== undefined) {
    throw new Error('Component instance "' + name + '" already defined.');
  }

  return {
    name: name,
    type: this._instances[name].type,
    version: this._instances[name].version,
    status: this._instances[name].instance !== null
  };
};
